...
$actionLogQuery = "SELECT id, username, action, log_date FROM action_log ORDER BY log_date DESC";
        ...
        <table>
            ...
            <tbody>
                ...
                if ($actionLogResult->num_rows > 0) {
                    while ($logRow = $actionLogResult->fetch_assoc()) {
                        echo '<tr>';
                        echo '<td>' . $logRow['action'] . '</td>';
                        echo '<td>' . $logRow['log_date'] . '</td>';
                        echo '</tr>';
                    }
                } else {
                    echo '<tr><td colspan="4">No action log available.</td></tr>';
                }
                ?>
            </tbody>
        </table>
...
<script>
    ...
    function updateReturnHistory(searchTerm, page) {
        ...
        fetch(`fetch_action_log_history.php?search=${searchTerm}&page=${page}`)
