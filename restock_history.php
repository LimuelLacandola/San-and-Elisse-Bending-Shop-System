...
// Query to retrieve data from restock_history table
$query = "SELECT * FROM restock_history
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<td>' . $row['quantity_restocked'] . '</td>';
                        echo '<td>' . $row['restock_date'] . '</td>';
                        echo '<td>' . $row['user_restocked'] . '</td>';
                        echo '</tr>';
                    }
                } else {
                    echo '<tr><td colspan="5">No restock history available.</td></tr>';
                }
                ?>
            </tbody>
        </table>
...
<script>
    // Function to update the restock history table with search and pagination
    function updateRestockHistory(searchTerm, page) {
        const transactionTable = document.querySelector('table tbody');

        // Send an AJAX request to fetch restock history data with search term and page
        fetch(`fetch_restock_history.php?search=${searchTerm}&page=${page}`)
    ...
    document.getElementById('searchBox').addEventListener('input', function () {
        ...
        updateRestockHistory(searchTerm, currentPage);
    });

    // Initial table update without search term
    updateRestockHistory('', currentPage);
</script>
</body>
</html>
